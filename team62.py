...
class Team62:
    ...
    def __init__(self):
        ...
        self.start_time = 0
        self.to_break = False
        self.block_weights = [pow(10, 3), pow(10, 4), pow(10, 5), pow(10, 6)]
        self.board_weights = [pow(10, 0), pow(10, 1), pow(10, 2), pow(20, 3)]
        #self.draw_weights = [pow(20, 3) / 6, pow(20, 3) / 9, pow(20, 3) / 12]
        # self.openmove_weight = pow(20,3)/4
        #
        self.plybonuscount=0
        self.oppbonuscount=0
    ...
    def move(self, board, old_move, flag):
        ...
        while time_elapsed < self.timeLimit:
            # iterative deepening
            #print(len(possible_moves),depth)
    ...
    def ids(self, board, old_move, depth):
        ...
        #    self.bonuscount=tempbonuscount
            self.plybonuscount=0
            next_move = moves
            update_status, block_won = board.update(old_move, next_move, self.player_num)
    ...
    def min_max(self, board, old_move, flag, depth, alpha, beta):
        ...
        if flag == self.player_num:
            ...
            #    self.bonuscount=tempbonuscount
                ...
                next_move = move
                update_status,block_won = board.update(old_move, next_move, self.opp)
                # openmove = board.block_status[old_move[0] % 4][old_move[1] % 4] != '-'
                # if board.block_status[next_move[0] / 4][next_move[1] / 4] != '-':
                #     if self.bonuscount == 0:
                #         self.bonuscount = 1
                ...
                #     self.bonuscount=0
                if block_won:
                    if self.oppbonuscount==0:
                        self.oppbonuscount=1
                        val = self.min_max(board,next_move,self.opp,depth-1,alpha,beta)
                        self.oppbonuscount=0
                    else:
                        self.oppbonuscount=0
